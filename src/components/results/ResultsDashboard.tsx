...
export default function ResultsDashboard({ result }: ResultsDashboardProps) {
  ...
  return (
    <div className="min-h-screen pb-16">
      {/* Header */}
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 glass">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          >
            ...
            <span className="font-bold text-slate-800 dark:text-slate-200 tracking-tight">HomeWise</span>
          </button>
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <h1 className="text-lg font-bold text-slate-800 dark:text-slate-200">Your Results</h1>
            <motion.button
              onClick={() => dispatch({ type: 'RESET' })}
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 rounded-xl
