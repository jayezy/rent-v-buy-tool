    ...
    return (
      <button
        ...
        className="w-full flex items-center justify-between py-3 px-1 rounded-xl
          hover:bg-slate-50 transition-colors cursor-pointer group/row text-left"
      >
        <span className="text-sm font-medium text-slate-500">{SHORT_LABELS[question.id] || question.id}</span>
        <span className="text-sm font-semibold text-slate-800 flex items-center gap-1.5">
          {formatAnswerDisplay(question, currentValue)}
          <svg
            className="w-3.5 h-3.5 text-slate-300 group-hover/row:text-slate-500 transition-colors"
    ...
    return (
      <div className="py-3 px-1">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-slate-500">{SHORT_LABELS[question.id]}</span>
          <button onClick={onCancelEdit} className="text-xs text-slate-400 hover:text-slate-600 cursor-pointer">Cancel</button>
        </div>
        <div className="flex gap-2 items-start">
          <div className="relative flex-1">
            <input
              ...
              autoFocus
              className={`w-full rounded-lg border py-2 px-3 text-sm text-slate-800 font-medium
          ...
          <button
            ...
            className="px-3 py-2 text-sm bg-slate-800 hover:bg-slate-900 disabled:bg-slate-200
              disabled:text-slate-400 text-white font-semibold rounded-lg transition-colors cursor-pointer
        ...
        {stateHint && !zipError && (
          <p className="mt-1 text-xs text-emerald-600 font-medium">{stateHint}</p>
        )}
        {zipError && (
          <p role="alert" className="mt-1 text-xs text-red-500">{zipError}</p>
        )}
      </div>
    )
  }
...
function CustomInlineInput({ config, onSubmit }: CustomInlineInputProps) {
  ...
  return (
    <div className="mt-2 flex gap-2 items-start">
      <div className="relative flex-1">
        {prefix && (
          <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs pointer-events-none">{prefix}</span>
        )}
        <input
          type="number"
          inputMode="decimal"
          ...
          placeholder="Custom"
          className={`w-full rounded-lg border py-1.5 text-xs text-slate-800 font-medium
        ...
        />
        {suffix && (
          <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs pointer-events-none">{suffix}</span>
        )}
      </div>
      <button
        onClick={handleSubmit}
        disabled={raw.trim() === ''}
        className="px-3 py-1.5 text-xs bg-slate-800 hover:bg-slate-900 disabled:bg-slate-200
          disabled:text-slate-400 text-white font-semibold rounded-lg transition-colors cursor-pointer
          disabled:cursor-not-allowed shrink-0"
      >
        Set
      </button>
      {error && <p role="alert" className="text-xs text-red-500 mt-0.5">{error}</p>}
    </div>
  )
}
